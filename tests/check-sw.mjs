...
function check() {
  ...
  // Приватность закреплена кодом, а не только текстом политики. Обе строки
  // легко потерять при рефакторинге, и потеря будет тихой: расширение
  // продолжит работать, просто начнёт слать cookie либо ходить на чужой хост.
  // Ищем по исходнику без комментариев, а не по `code`: stripNonCode вырезает
  // строковые литералы, а искомое значение как раз литерал. Комментарии всё
  // равно убираем — упоминание в комментарии проверкой считаться не должно.
  if (!/credentials\s*:\s*["']omit["']/.test(stripComments(swSource))) {
    problems.push(
      `${swName}: в запросе ленты нет credentials: "omit" — cookie пользователя уйдут на портал. Это обещано в docs/privacy-policy.md.`
    );
  }
  // Ищем именно ВЫЗОВ со скобкой: голое имя есть ещё и в списке импорта,
  // поэтому проверка на идентификатор пропускала удаление самой проверки.
  if (!/\bisAllowedFeedUrl\s*\(/.test(code)) {
    problems.push(
      `${swName}: нет проверки isAllowedFeedUrl перед запросом — адрес из настроек уйдёт на любой сайт. host_permissions это не блокирует.`
    );
  }
